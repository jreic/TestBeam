...
filepath = "/uscms/home/joeyr/nobackup/realign_MJ531_spill_by_spill/"
outname = "MJ531_run_by_run_angle_scan"

runNumbers = [
46657, 46658, 46659, 46660, 46662, 46665, 46666, 46667, 
46668, 46669, 46670, 46671, 46672, 46673, 46674, 46675,
46676, 46677, 46678, 46679, 46680, 46681, 46682, 
46683, 46684, 46685, 46687, 46688, 46689, 46690, 
46691, 46692, 46693, 46694, 46695, 46696, 46697, 
46698, 46699, 46700, 46701, 46702, 46703, 46704, 
46705, 46706, 46707, 46708, 46709, 46710, 46711, 
46712, 46713, 46715]


#46657, 46658, 46659, 46660, 46662, 46664, 46665, 46666, 46667, 46668, 46669, 46670, 46671, 46672, 46673, 46674, 46675, 46676, 46677, 46678, 46679, 46680, 46681, 46682, 46683, 46684, 46685, 46687, 46688, 46689, 46690, 46691, 46692, 46693, 46694, 46695, 46696, 46697, 46698, 46699, 46700, 46701, 46702, 46703, 46704, 46705, 46706, 46707, 46708, 46709, 46710, 46711, 46712, 46713, 46714, 46716]
#46657, 46658, 46659, 46660, 46662, 46664, 46665, 46666, 46667]
...
for plot_path in plot_paths :
    ...
    elif "Residual" in prefix :
        plot_name = prefix+"_sigma"

    h_rms = h_mean.Clone(plot_name)
    h_rms.SetTitle(plot_name)
    h_rms.GetYaxis().SetTitle("max uncert" if "Efficiency" in plot_name else "sigma" if "Residual" in plot_name else "rms")
...
for i_runNumber, runNumber in enumerate(runNumbers) :
    ...
    for i_plot_path, plot_path in enumerate(plot_paths) :
        ...
        if "Efficiency" in plot_path :
            ...
            output_plots_mean[i_plot_path].SetBinContent(ibin, h.GetMean())
            output_plots_rms[i_plot_path].SetBinContent(ibin, sigma)
            output_plots_rms[i_plot_path].SetBinError(ibin, sigma_err)
            output_plots_rms[i_plot_path].GetYaxis().SetRangeUser(0,12.8)
