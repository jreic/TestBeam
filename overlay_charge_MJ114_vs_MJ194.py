...
ps = plot_saver(plot_dir("overlay_charge_MJ114_vs_MJ194"), size=(600,600), log=True, pdf=True, pdf_log=True)
...
for plot_name in plot_names :
    ...
    fpath114 = "/uscms/home/joeyr/publicweb/TFPX/Chewie_Runs32479_32499/"+plot_name+".root"
    ...
    files = [ ROOT.TFile(fpath114)
             ,ROOT.TFile(fpath194)
            ]

    labels = ["114 (CNM, -51V)","194 (FBK, -30V)"]

    colors = [ROOT.kRed,ROOT.kBlue]
    hists = []

    if "Landau" in plot_name :
        leg = ROOT.TLegend(0.55, 0.75, 0.90, 0.90)
    else :
    ...
    for index in xrange(0,len(files)) :
        ...
        if "Landau" in plot_name and h.GetEntries() > 0 :
            ...
            fit = h.Fit(langaus, "RBLSQ", "", 8000, 25000)
